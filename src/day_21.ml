...
let parse_parens (s : string) (i : int) : string list =
    let rec loop (xs : string list) (i : int) (j : int) : string list =
        match s.[j] with
            | ')' -> push xs s i j |> List.filter ((<>) "contains")
            | ',' | ' ' ->
...
let reduce (table : (string, StrSet.t) Hashtbl.t) (key : string) : unit =
...
let () : unit =
    ...
    (
        ...
        let xs : string list =
            List.filter (fun x -> StrSet.mem x union |> not) xs in
        List.length xs |> Printf.printf "%d\n"
    );
    (
        let keys_values : (string * string) list =
            Hashtbl.to_seq table
            |> List.of_seq
            |> List.map (fun (k, v) -> (k, StrSet.choose v))
            |> List.sort (fun (a, _) (b, _) -> compare a b) in
        List.iter (fun (_, value) -> Printf.printf "%s," value) keys_values;
        Printf.printf "\n"
    )
